<template>
  <div>
    <h2 id="page-heading" data-cy="BookHeading">
      <span v-text="t$('novelsyncApp.book.home.title')" id="book-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" @click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('novelsyncApp.book.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'BookCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-book">
            ...
            <span v-text="t$('novelsyncApp.book.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && books && books.length === 0">
      <span v-text="t$('novelsyncApp.book.home.notFound')"></span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" @click="changeOrder('title')">
              <span v-text="t$('novelsyncApp.book.title')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" @click="changeOrder('isbn')">
              <span v-text="t$('novelsyncApp.book.isbn')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'isbn'"></jhi-sort-indicator>
            </th>
            <th scope="row" @click="changeOrder('price')">
              <span v-text="t$('novelsyncApp.book.price')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'price'"></jhi-sort-indicator>
            </th>
            <th scope="row" @click="changeOrder('stockQuantity')">
              <span v-text="t$('novelsyncApp.book.stockQuantity')"></span>
            ...
            <th scope="row" @click="changeOrder('imageUrl')">
              <span v-text="t$('novelsyncApp.book.imageUrl')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'imageUrl'"></jhi-sort-indicator>
            </th>
            <th scope="row" @click="changeOrder('category.categoryName')">
              <span v-text="t$('novelsyncApp.book.category')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'category.categoryName'"></jhi-sort-indicator>
            </th>
            <th scope="row" @click="changeOrder('author.authorName')">
              <span v-text="t$('novelsyncApp.book.author')"></span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'author.authorName'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
          ...
          <tr v-for="book in books" :key="book.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="book.category">
                <router-link :to="{ name: 'CategoryView', params: { categoryId: book.category.id } }">{{
            ...
            <td>
              <div v-if="book.author">
                <router-link :to="{ name: 'AuthorView', params: { authorId: book.author.id } }">{{ book.author.authorName }}</router-link>
              </div>
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="novelsyncApp.book.delete.question" data-cy="bookDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-book-heading" v-text="t$('novelsyncApp.book.delete.question', { id: removeId })"></p>
      </div>
