...
public interface BookRepository extends JpaRepository<Book, Long>, JpaSpecificationExecutor<Book> {
    ...
    @Query(
        value = "select book from Book book left join fetch book.category left join fetch book.author",
        countQuery = "select count(book) from Book book"
    )
    Page<Book> findAllWithToOneRelationships(Pageable pageable);

    @Query("select book from Book book left join fetch book.category left join fetch book.author")
    List<Book> findAllWithToOneRelationships();

    @Query("select book from Book book left join fetch book.category left join fetch book.author where book.id =:id")
